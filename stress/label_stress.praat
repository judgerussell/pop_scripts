...
#
#
# edited to work for my purpose > : ) - judge russell 8.14.2020
# those are, labeling multiple lexical stress textgrid tiers of lyrics
# added a check for the correct amount of lines per tier
# if there's a blank line while its labeling a tier, it gives up. if there is a non blank line after the tier ends, it gives up.
# as well this labels a sequence of tiers, starting at Tier and ending at Tier + Tier_count
...
sentence Filename D:\Classes\2020summer\research\top5past5\lyrics\unformatted_stress\formatted_stress\old_town_road.txt_stress.txt_formatted.txt
comment Which IntervalTier in the selected TextGrid do you want to start with?
integer Tier_(index) 1 (= the first IntervalTier)
comment How many vocal tracks are there?
integer Tier_count 1
comment Which interval do we start labeling from?
integer Starting_interval_(index) 2 (= the first interval)
...
for i from tier to tier + tier_count
	...
	for interval from starting_interval to numberOfIntervals
        ...
        if newlabel$ == "\n"
            exit Labels  are not consistent with intervals.
        endif
		...
		# we add the new label we collected from the text file:
			if overwrite = 1
						Set interval text... i interval 'newlabel$'
					elsif oldlabel$ = ""
					Set interval text... i interval 'newlabel$'
					else 
